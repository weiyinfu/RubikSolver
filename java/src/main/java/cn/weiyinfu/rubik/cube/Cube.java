...
public class Cube {
    ...
    int N;//魔方的阶数
    public int[][][] colors;//剑形表示法，第一维表示六个面，第二维表示x，y
    ...
    public Cube(String s) {
        ...
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < n; j++) {
                for (int k = 0; k < n; k++) {
                    this.colors[i][j][k] = COLOR.indexOf(s.charAt(si++));
                }
            }
        }
    }

    void rotateFace(int[][] a, boolean reverse) {
        var b = new int[N][N];
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                b[i][j] = a[i][j];
...
//edge表示face的哪一条边跟中间是邻居
    ...
    public Cube regular() {
        ...
        for (int i = 0; i < 24; i++) {
            ...
            int white = COLOR.indexOf('w');
            int red = COLOR.indexOf('r');
            int blue = COLOR.indexOf('b');
            if (b.colors[0][b.N - 1][b.N - 1] == white && b.colors[2][0][b.N - 1] == red && b.colors[3][0][0] == blue) {
                return b;
            }
        }
    ...
    public String toCompactString() {
        ...
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < N; j++) {
                for (int k = 0; k < N; k++) {
                    builder.append(COLOR.charAt(colors[i][j][k]));
                }
            }
        }
